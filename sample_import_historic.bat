...
%PROPY% %BUILDINGS%delete.py %TARGETFC% && (
  echo deleted target %TARGETFC% on %date% at %time% >> %TARGETLOGDIR%import_historic.log
) || (
...
%PROPY% %BUILDINGS%import.py %TARGETFC% %SOURCEFC% %EDITORS% && (
  echo imported %TARGETFC% on %date% at %time% >> %TARGETLOGDIR%import_historic.log
) || (
...
%PY27% %TOILER%\src\py27\create_versionedviews.py %TARGETFC% && (
  echo created versioned view for %TARGETFC% on %date% at %time% >> %TARGETLOGDIR%import_historic.log
) || (
  %PROPY% %BUILDINGS%notify.py ": Failed to create versioned view for %TARGETFC% on %SDEFILE%" %NOTIFY% && EXIT /B 1
) 
%PROPY% %BUILDINGS%export.py %TARGETFC% && (
  echo expoorted %TARGETFC% on to json on %date% at %time% >> %TARGETLOGDIR%import_historic.log
) || (
  %PROPY% %BUILDINGS%notify.py ": Failed to export %TARGETFC% to json" %NOTIFY% && EXIT /B 1
)  
%PROPY% %BUILDINGS%notify.py ": Successfully imported %TARGETFC% to %SDEFILE%" %NOTIFY%
